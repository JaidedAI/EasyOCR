...
def consecutive(data: np.ndarray, mode: str = "first", stepsize: int = 1) -> List:
    """[summary] # TODO

    Args:
        data (np.ndarray): [description]
        mode (str, optional): [description]. Defaults to "first".
        stepsize (int, optional): [description]. Defaults to 1.

    Returns:
        List: [description] # TODO: ghandic - unsure on response shape
    """
...
def word_segmentation(
    ...
    Args:
        ...
        separator_idx (Dict[str, optional): [description]. Defaults to {"th": [1, 2], "en": [3, 4]}.
        separator_idx_list (List[int], optional): [description]. Defaults to [1, 2, 3, 4].
...
class BeamState:
    ...
    def wordsearch(self, classes: List[str], ignore_idx: List[int], maxCandidate: int, dict_list: List[str]) -> str:
        """[summary] # TODO

        Args:
            classes (List[str]): [description]
            ignore_idx (List[int]): [description]
            maxCandidate (int): [description]
            dict_list (List[str]): [description]
...
class LanguageModel:
    ...
    https://github.com/ThomasDelteil/HandwrittenTextRecognition_MXNet/blob/language_model/utils/CTCDecoder/LanguageModel.py
    """

    def __init__(self, fn: str, classes: List[str]):
        "read text from file to generate language model"
        self.initWordList(fn)
        self.initCharBigrams(fn, classes)

    def initWordList(self, fn: str):
        "internal init of word list"
        txt = open(fn).read().lower()
        words = re.findall(r"\w+", txt)
        self.words = list(filter(lambda x: x.isalpha(), words))

    def initCharBigrams(self, fn: str, classes: List[str]):
    ...
    def getCharBigram(self, first: str, second: str) -> float:
...
def applyLM(parentBeam: BeamEntry, childBeam: BeamEntry, classes: List[BeamEntry], lm: LanguageModel):
    """calculate LM score of child beam by taking score from parent beam and bigram probability of last two chars
    # TODO

    Args:
        parentBeam (BeamEntry): [description]
        childBeam (BeamEntry): [description]
        classes (List[BeamEntry]): [description]
        lm (LanguageModel): [description]
    """
...
def simplify_label(labeling: List[str], blankIdx: int = 0) -> Tuple:
    """[summary] # TODO

    Args:
        labeling (List[str]): [description]
        blankIdx (int, optional): [description]. Defaults to 0.

    Returns:
        Tuple: [description] # TODO: ghandic - unsure on response shape
    """
...
def addBeam(beamState: BeamState, labeling: str):
    ...
    Args:
        beamState (BeamState): [description]
        labeling (str): [description]
    """
...
def ctcBeamSearch(
    ...
    Args:
        ...
        ignore_idx (List[int]): [description]
        lm (LanguageModel): [description]
        beamWidth (int, optional): [description]. Defaults to 25.
        dict_list (List[str], optional): [description]. Defaults to [].
...
class CTCLabelConverter(object):
    ...
    def __init__(self, character: str, separator_list: Dict[str, str] = {}, dict_pathlist: Dict[str, str] = {}):
    ...
    def encode(self, text: str, batch_max_length: int = 25) -> Tuple[torch.IntTensor, torch.IntTensor]:
        """convert text-label into text-index.

        Args:
            text (str): text labels of each image. [batch_size]
            batch_max_length (int, optional): [description]. Defaults to 25.

        Returns:
            Tuple[torch.IntTensor,torch.IntTensor]: concatenated text index for CTCLoss, length of each text
        """
    ...
    def decode_greedy(self, text_index: str, length: int) -> List[str]:
        """Converts text-index into text-label.
        # TODO

        Args:
            text_index (str): [description]
    ...
    def decode_beamsearch(self, mat: np.ndarray, beamWidth: int = 5) -> List[str]:
        """[summary] # TODO
    ...
    def decode_wordbeamsearch(self, mat: np.ndarray, beamWidth: int = 5) -> List[str]:
        """[summary] # TODO
...
def four_point_transform(
    image: np.ndarray, rect: Tuple[Tuple[float, float], Tuple[float, float], Tuple[float, float], Tuple[float, float]]
) -> np.ndarray:
    """[summary] # TODO

    Args:
        image (np.ndarray): [description]
        rect (Tuple[Tuple[float, float], Tuple[float, float], Tuple[float, float], Tuple[float, float]]): [description]
...
def group_text_box(
    ...
    Args:
        polys (List[Tuple[float, float]]): [description]
        ...
        width_ths (float, optional): [description]. Defaults to 1.0.
        add_margin (float, optional): [description]. Defaults to 0.05.

    Returns:
        Tuple[List,List]: [description] # TODO: ghandic - unsure on response shape
    """
...
def get_image_list(horizontal_list: List, free_list: List, img: np.ndarray, model_height: int = 64) -> Tuple[List, int]:
    """[summary] TODO: ghandic - Unsure on shape of inputs here...

    Args:
        horizontal_list (List): [description]
        free_list (List): [description]
        img (np.ndarray): [description]
        model_height (int, optional): [description]. Defaults to 64.

    Returns:
        Tuple[List, int]: [description] # TODO: ghandic - unsure on response shape
    """
    image_list = []
...
def download_and_unzip(url: str, filename: str, model_storage_directory: str):
    """[summary] # TODO

    Args:
        url (str): [description]
        filename (str): [description]
        model_storage_directory (str): [description]
    """
...
def calculate_md5(fname: str) -> str:
    """[summary] # TODO
...
def get_paragraph(
    ...
    Args:
        raw_result (List): [description]
        x_ths (float, optional): [description]. Defaults to 1.
