...
class VGG_FeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channel, output_channel=512, direction='Horizontal'):
        super(VGG_FeatureExtractor, self).__init__()
        self.kernel_and_stride = (2,1) if direction == 'Horizontal' else (1,2)
        ...
        self.ConvNet = nn.Sequential(
            ...
            nn.MaxPool2d(self.kernel_and_stride, self.kernel_and_stride),  # 256x4x25
            nn.Conv2d(self.output_channel[2], self.output_channel[3], 3, 1, 1, bias=False),
            nn.BatchNorm2d(self.output_channel[3]), nn.ReLU(True),  # 512x4x25
            nn.Conv2d(self.output_channel[3], self.output_channel[3], 3, 1, 1, bias=False),
            nn.BatchNorm2d(self.output_channel[3]), nn.ReLU(True),
            nn.MaxPool2d(self.kernel_and_stride, self.kernel_and_stride),  # 512x2x25
