...
def build_optimizer(opt, filtered_parameters):
    if opt.optim == 'adam':
...
def training_step(model, optimizer, criterion, train_dataset, converter, opt, amp: bool, scaler):
    if amp:
        with autocast():
...
def train(opt, show_number=2, amp=False):
    ...
    # freeze some layers
    filtered_parameters = freeze_model(model, opt)

    # setup optimizer
    optimizer = build_optimizer(opt, filtered_parameters)
    ...
    while (True):
        ...
        cost = training_step(model, optimizer, criterion, train_dataset, converter, opt, amp, scaler)
